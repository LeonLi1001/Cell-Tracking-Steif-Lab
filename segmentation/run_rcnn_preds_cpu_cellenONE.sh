...
print_run_name="htert_20230822_131349_843.Run"   # Pass print run name as the second command line argument

cd /projects/steiflab/scratch/leli/Segmentation
#mkdir /projects/steiflab/scratch/leli/slurm_output/A118880/$print_run_name 
...
image_directory="/projects/steiflab/archive/data/imaging/${chip_name}/CellenONEImages/${print_run_name}"

for image_filepath in ${image_directory}/*; do
    
    # Submit a job for each image
    sbatch --job-name=rcnn_pred_img --output="/projects/steiflab/scratch/leli/slurm_output/$chip_name/$print_run_name/$(basename $image_filepath).o" --wrap="python PredictRCNNCPU_cellenONE.py $image_filepath $chip_name $print_run_name"
